...
const Apostilas = () => {
  ...
  // ─── Carrega PDFs do contrato (apostilas) ───
  useEffect(() => {
    let active = true;
    apiService.getDocumentosContrato()
      .then((data) => {
        if (!active) return;
        setDocsContrato((data?.documentos || []).filter(d => d.grupo === 'apostilas'));
      })
  ...
  // Mapa de documentos por SEI e por número da apostila
  ...
  const findDoc = (a) => {
    ...
    const numApostila = (a.N_DO_ADITIVO || '').split(' ')[0];
    if (numApostila && docsMap.has(numApostila)) return docsMap.get(numApostila);
    return null;
  };
