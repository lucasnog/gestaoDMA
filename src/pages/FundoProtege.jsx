...
import api, {
  conferirFundoProtege, downloadConferenciaFundoProtege, listarRelatoriosFundoProtege
...
function PontoAccordion({
  ...
  detalhamentos, statusPorSemana, municipiosSemana, datasChegada,
  ...
  const isPatrulha = segmento === 'GMPK';
  const ultimoFisico = fisicos[fisicos.length - 1];
  const ordemReversa = semanas.map((_, i) => i).reverse();
      ...
      >
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            className={`p-1 rounded-lg transition-colors ${ativo ? 'text-emerald-600 bg-emerald-50 hover:bg-emerald-100' : 'text-slate-300 bg-slate-50 hover:bg-slate-100'}`}
          >
            {ativo ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
          </button>
          <div className={`p-1 rounded-lg transition-colors ${aberto ? 'bg-emerald-100 text-emerald-600' : 'text-slate-300'}`}>
        ...
        <div className="border-t border-slate-100">
          <div className="px-4 pt-4 pb-1 space-y-0">
            ...
            {ordemReversa.map((i) => {
              ...
              return (
                <div key={sem} className={`relative flex gap-4 pb-6 last:pb-0 ${i < semanas.length - 1 ? 'pt-1' : ''}`}>
                  ...
                  <div className={`flex-1 min-w-0 -mt-1 p-3 rounded-xl ${isAtual ? 'bg-emerald-50/50 ring-1 ring-emerald-100' : 'bg-white border border-slate-100'}`}>
                    <div className="flex items-center flex-wrap gap-2 mb-2">
                      <span className={`text-xs font-bold ${isAtual ? 'text-emerald-700' : 'text-slate-500'}`}>
                        {sem.replace('/2026', '')}
                      </span>
                      {isAtual && (
                        <span className="text-[9px] font-bold text-white bg-emerald-500 px-1.5 py-0.5 rounded-md uppercase">Atual</span>
                      )}
                      {st && st !== '-' && (
                    ...
                    {det && det !== '-' && (
                      <div className="text-sm text-slate-700 leading-relaxed p-3 rounded-lg bg-blue-50/60 border border-blue-100/50">
                        ...
                        {det.split(/\r?\n/).filter(Boolean).map((linha, li) => {
                          ...
                          if (partes.length >= 2) {
                            const fase = partes.slice(0, -1).join(' - ');
                            ...
                            const cor = situacao === 'Aprovado' || situacao === 'Aprovado pela analista' || situacao === 'Finalizado' ? 'emerald'
                              : situacao === 'Em analise' || situacao === 'Em Andamento' ? 'blue'
...
function ContratoCard({ contrato, idx, semanas, semanaMaisRecente, toggleContrato, aberto, detalhe }) {
  ...
  const pontosPorNome = useMemo(() => {
    ...
    if (detalhe?.pontos) {
      for (const p of detalhe.pontos) {
        if (!map[p.ponto]) map[p.ponto] = {};
        map[p.ponto][p.semana] = {
  ...
  const statusInfo = getStatusFromProgress(recente?.fisico);

  const isPontoAtivo = (nome) => !pontosAtivos || pontosAtivos.has(nome);

  const togglePonto = (nome) => {
    setPontosAtivos(prev => {
      if (!prev) return new Set([nome]);
  ...
  const pontosGrafico = useMemo(() => {
    ...
    return nomesPontos.filter(n => pontosAtivos.has(n));
          ...
          <div className="flex items-center gap-4 shrink-0">
            {recente && (
              <div className="text-center">
                <div className="text-[10px] text-emerald-500 font-semibold mb-1">{semanaMaisRecente?.replace('/2026', '')}</div>
                <ProgressRing value={recente.fisico} size={48} stroke={4} />
              </div>
            )}

            <div className={`p-1.5 rounded-lg transition-colors ${aberto ? 'bg-emerald-100 text-emerald-600' : 'text-slate-300'}`}>
          ...
          {nomesPontos.length > 0 && (
            <>
                  ...
                  <div className="flex gap-1">
                    ...
                    {pontosAtivos && (
                      <button
                        ...
                        className="ml-1 inline-flex items-center gap-1 px-2.5 py-1 text-[10px] font-semibold rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-colors"
                  ...
                  <ResponsiveContainer width="100%" height={260}>
                    <LineChart data={(() => {
                      ...
                      return semLabels.map((sem, i) => {
                        const o = { semana: sem };
                        for (const nome of pontosGrafico) {
                          const d = pontosPorNome[nome];
                          const vals = semLabels.map((_, j) => {
                            const p = d?.[semanas[j]];
                            return p?.[metricKey] != null ? Number(p[metricKey]) : null;
                          });
                          o[nome] = fillProgress(vals)[i] ?? 0;
                        }
                        return o;
                      });
                    })()}>
                      ...
                      <Legend
                        ...
                        iconType="circle"
                        payload={nomesPontos.map((nome, idx) => ({
                          value: nome.replace('Ponto ', ''),
                          dataKey: nome,
                          color: CORES_PONTOS[idx % CORES_PONTOS.length],
                          type: 'circle',
                          inactive: pontosAtivos != null && !pontosAtivos.has(nome),
                        }))}
                        onClick={(entry) => {
                      ...
                      {pontosGrafico.map((nome, idx) => (
                        <Line key={nome} type="monotone" dataKey={nome} stroke={CORES_PONTOS[idx % CORES_PONTOS.length]} strokeWidth={2} dot={false} name={nome.replace('Ponto ', '')} connectNulls />
                      ))}
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {pontosAtivos && (
                    <button
                      onClick={() => setPontosAtivos(null)}
                      className="ml-auto inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md hover:bg-emerald-100 transition-colors"
                    >
              ...
              <div className="space-y-2">
                {nomesPontos.map(nome => {
                  const dados = pontosPorNome[nome];
                  ...
                  return (
                    <PontoAccordion
                      ...
                      ativo={isPontoAtivo(nome)}
                      onToggleAtivo={() => togglePonto(nome)}
                    />
                  );
                })}
              </div>
...
function ConferenciaFundoProtege() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await listarRelatoriosFundoProtege();
  ...
  async function processar() {
    ...
    setResultado(null);
    try {
      const res = await conferirFundoProtege(
  ...
  async function baixarDocx() {
    if (!resultado) return;
    try {
      await downloadConferenciaFundoProtege(
        oldFile.semana_id ? { semana_id: oldFile.semana_id } : oldFile.file,
        newFile.semana_id ? { semana_id: newFile.semana_id } : newFile.file,
        resultado.fileName,
      );
  ...
  return (
    <Card className="!p-6 border-emerald-100">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-sm font-bold text-slate-800">Conferência Fundo Protege</h3>
      ...
      <div className="flex flex-wrap items-center gap-2 mt-3">
        <button
          ...
          className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition-colors ${
            !usarUpload ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
          }`}
        >
          Relatórios do banco
        </button>
      ...
      {!usarUpload ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div className={`p-4 rounded-xl border-2 ${oldFile ? 'border-emerald-300 bg-emerald-50/40' : 'border-slate-200 bg-slate-50'}`}>
            ...
            <select
              ...
              className="w-full text-sm rounded-lg border border-slate-200 px-3 py-2 bg-white focus:ring-2 focus:ring-emerald-200 focus:border-emerald-400 outline-none"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <label className={`flex items-center gap-3 p-4 rounded-xl border-2 border-dashed cursor-pointer transition-colors ${
            oldFile ? 'border-emerald-300 bg-emerald-50/40' : 'border-slate-200 bg-slate-50 hover:border-emerald-300'
          }`}>
          ...
          <label className={`flex items-center gap-3 p-4 rounded-xl border-2 border-dashed cursor-pointer transition-colors ${
            newFile ? 'border-emerald-300 bg-emerald-50/40' : 'border-slate-200 bg-slate-50 hover:border-emerald-300'
          }`}>
      ...
      <div className="flex items-center gap-3 mt-4">
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
...
<td className="px-3 py-2 text-right">
                          <button
                            ...
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-bold transition-colors ${
                              r.num_changes > 0 ? 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100' : 'bg-slate-50 text-slate-300'
                            }`}
                          >
                            {r.num_changes}
                          </button>
                    ...
                    return (
                      <div key={i} className="px-4 py-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {partes.length > 0 && <span className="text-[11px] text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md font-semibold">{partes.join(' · ')}</span>}
                        </div>
...
export default function FundoProtege() {
  ...
  async function loadData() {
    try {
      setLoading(true);
      const res = await api.get('/fundo-protege/contratos');
      setData(res.data);
    } catch (err) {
  ...
  async function toggleContrato(contrato) {
    ...
    if (!aberto && !detalhes[contrato]) {
      try {
        const res = await api.get(`/fundo-protege/contratos/${encodeURIComponent(contrato)}`);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <div className="p-2.5 bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-xl shadow-sm shadow-emerald-200">
              <Shield className="w-5 h-5 text-white" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-slate-800">Fundo Protege</h1>
      ...
      <div className="flex items-center gap-1 border-b border-slate-200 pb-px">
        ...
        >
          <ClipboardCheck className="w-4 h-4" /> Conferência Fundo Protege
        </button>
      </div>

      {aba === 'conferencia' ? (
        <ConferenciaFundoProtege />
      ) : (
      <>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
        <Card className="!p-5">
